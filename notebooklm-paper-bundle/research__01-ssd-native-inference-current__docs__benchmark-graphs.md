...
8. Correctness parity is now a release gate: session-tail kept most selected IFEval cases but failed every selected GraphWalks case.
...
## Correctness Parity By Task

![Correctness parity by task](assets/benchmark-graphs/correctness-parity-by-task.svg)

The correctness result is the reason this project moved toward quality-gated stateful KV reuse. The full-prompt gate selected 42 passing cases; session-tail passed 31/42 overall, with 31/36 selected IFEval cases passing and 0/6 selected GraphWalks cases passing.

The chart is not an argument against caching. It is evidence that reuse policy must be task-aware and quality-gated before session-tail or restored-prefix execution can be treated as a general replacement for full-prompt execution.
