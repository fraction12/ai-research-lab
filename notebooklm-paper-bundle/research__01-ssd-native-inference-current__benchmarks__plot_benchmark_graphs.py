...
CORRECTNESS_PARITY_SUMMARY = ROOT / "benchmarks/datasets/flashcache-correctness-parity-2026-06-03/summary.json"
...
def write_chart_data() -> None:
    ...
    correctness = load_json(CORRECTNESS_PARITY_SUMMARY)
    correctness_rows: list[dict[str, Any]] = []
    for dataset_id, row in sorted(correctness["parity"]["by_dataset"].items()):
        correctness_rows.append(
            {
                "dataset_id": dataset_id,
                "selected_count": row["selected_count"],
                "session_tail_pass_count": row["session_tail_pass_count"],
                "session_tail_pass_rate": row["session_tail_pass_rate"],
                "mean_session_tail_score": row["mean_session_tail_score"],
            }
        )
    correctness_rows.append(
        {
            "dataset_id": "overall",
            "selected_count": correctness["parity"]["summary"]["session-tail"]["case_count"],
            "session_tail_pass_count": round(
                correctness["parity"]["summary"]["session-tail"]["case_count"]
                * correctness["parity"]["summary"]["session-tail"]["pass_rate"]
            ),
            "session_tail_pass_rate": correctness["parity"]["summary"]["session-tail"]["pass_rate"],
            "mean_session_tail_score": correctness["parity"]["summary"]["session-tail"]["mean_score"],
        }
    )
    payload = {
        ...
        "correctness_parity": correctness_rows,
        "correctness_prompt_time_savings_ms": correctness["prompt_time_savings_ms"],
    }
...
def correctness_parity_chart() -> list[Path]:
    summary = load_json(CORRECTNESS_PARITY_SUMMARY)
    dataset_order = ["ifeval", "graphwalks", "overall"]
    labels = ["IFEval", "GraphWalks", "Overall"]
    rates: list[float] = []
    counts: list[int] = []
    pass_counts: list[int] = []
    for dataset_id in dataset_order:
        if dataset_id == "overall":
            total = int(summary["parity"]["summary"]["session-tail"]["case_count"])
            rate = float(summary["parity"]["summary"]["session-tail"]["pass_rate"])
            passed = round(total * rate)
        else:
            row = summary["parity"]["by_dataset"][dataset_id]
            total = int(row["selected_count"])
            rate = float(row["session_tail_pass_rate"])
            passed = int(row["session_tail_pass_count"])
        rates.append(rate * 100)
        counts.append(total)
        pass_counts.append(passed)

    fig, ax = plt.subplots(figsize=(7.2, 4.45), constrained_layout=False)
    fig.subplots_adjust(left=0.11, right=0.975, bottom=0.25, top=0.74)
    annotate_title(
        fig,
        "Session-tail speed is not yet quality-safe",
        "Full-prompt gate selected 42 passing cases; session-tail retained IFEval but collapsed on GraphWalks.",
    )
    colors = [COLORS["cache"], COLORS["restart"], COLORS["gray"]]
    ax.bar(labels, rates, color=colors, width=0.58)
    ax.axhline(100, color=COLORS["muted"], linewidth=0.8, linestyle=(0, (4, 3)))
    ax.text(2.0, 97.5, "Full prompt selected set = 100%", ha="center", va="top", fontsize=7.5, color=COLORS["muted"])
    ax.set_ylim(0, 112)
    ax.set_ylabel("Session-tail pass rate")
    finish_axes(ax, FuncFormatter(percent_label))
    for idx, (rate, passed, total) in enumerate(zip(rates, pass_counts, counts)):
        y = rate + 3.0 if rate > 4 else 5.0
        ax.text(idx, y, f"{passed}/{total}\n{rate:.1f}%", ha="center", va="bottom", fontsize=8.2)
    mean_saved = float(summary["prompt_time_savings_ms"]["mean"])
    fig.text(
        0.11,
        0.055,
        f"Mean selected-case prompt-time saving: {mean_saved:.0f} ms. Quality drift is now a release gate, not a footnote.",
        ha="left",
        va="bottom",
        fontsize=7.5,
        color=COLORS["muted"],
    )
    return save_figure(fig, "correctness-parity-by-task")
