...
\title{Attention-State Reuse for Local Tool-Using Agents}
...
The evaluation uses BFCL-derived tasks \cite{bfcl2025} with a Gemma 4 12B model blob installed through Ollama and executed for the KV lanes via a llama.cpp CUDA sequence-file route \cite{llamacpp}. The goal is not to compare against hosted frontier systems. The goal is to isolate whether runtime state and tool-interface design can make a local model function better and more reliably on repeated tool-use work.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item BFCL-derived results showing restored KV Capsule behavior matching native live append under negative controls.
...
Restored KV Capsule matched both full visible PTI and native live append at 100/100. Native/restored hash parity was also 100/100, supporting the claim that restored hidden state preserved the useful stable-prefix behavior for these cases. Table~\ref{tab:mechanism} summarizes the positive mechanism lanes.
...
Both negative controls stayed closed with zero leaks. The fresh-tail lane shows that task tails alone did not contain enough visible information to solve the selected cases. The wrong-capsule lane shows that irrelevant hidden state did not substitute for the correct stable prefix.
...
\caption{Repeated-work systems comparison. This is a practical runtime comparison, not a prompt-identical KV-versus-compaction mechanism comparison.}
...
The evaluation is scoped to local Gemma 4 12B experiments. The text-threaded baseline is a practical runtime comparison, not a prompt-identical mechanism comparison. The selected cohort is quality-gated and BFCL-derived; it is not an official BFCL leaderboard submission. PTI is a structured local tool interface for agent runtimes, not arbitrary programmatic tool calling. Baseline visible-input telemetry is cumulative reported burden, not clean per-task accounting. The selected-cohort mechanism run does not show restored KV is faster than native append. Fresh-tail and wrong-capsule controls test leakage and prefix dependence only on this selected cohort; they do not establish general safety of hidden-state reuse. The 89 compaction events are supported by tracked summary artifacts, but the raw Windows Codex session JSONL is not present in this checkout. The compaction baseline is diagnostic for practical repeated-work overhead, not a causal analysis of which individual summaries helped or hurt.
...
All paper-facing artifacts, analysis scripts, figure manifests, and arXiv sources are available in the public repository at \url{https://github.com/fraction12/ai-research-lab}. The selected-cohort mechanism results are under \path{research/02-quality-gated-stateful-kv-reuse/experiments/paper-grade-code-mode-kv-capsule-evaluation-2026-06-05/}, including \path{selected-cohort-summary.json} and \path{selected-cohort-findings.md}. The repeated-work comparison is in the same directory, including \path{repeated-work-speed-summary.json} and \path{repeated-work-speed-findings.md}. The BFCL calibration artifacts are under \path{research/02-quality-gated-stateful-kv-reuse/experiments/bfcl-official-kv-pti-leaderboard-lane/}, including \path{official-evaluator-summary-2026-06-07.md}, \path{official-context-burden-summary-2026-06-07.md}, and \path{official-latency-derivation-2026-06-07.md}. Figure provenance is recorded in \path{research/02-quality-gated-stateful-kv-reuse/paper-artifacts/figures/figure-manifest.json}, and the exact LaTeX source for this draft is under \path{research/02-quality-gated-stateful-kv-reuse/arxiv/}.
