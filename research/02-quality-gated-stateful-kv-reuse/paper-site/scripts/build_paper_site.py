...
initial_official_bfcl_rows = [
    ...
    ("scoreable generated-record count", 969, 1390, "69.71%"),
]
official_bfcl_reference_rows = [
    (
        ...
        "N/A",
        "69.88%",
        "45,332 fresh visible task-tail tokens after restore; 32.6 mean",
...
<body>
  ...
  <main>
    ...
    <section id="abstract">
      <h2>Abstract</h2>
      <div class="abstract">
        Local tool-using agents repeatedly expose stable context to the model: tool schemas, runtime rules, interface contracts, and task protocols. Text compaction preserves continuity by writing summaries back into the prompt; attention-state reuse preserves a different object, the model's computed key/value state after reading a stable prefix. We evaluate <strong>KV Capsules</strong>, saved and restored KV/sequence state for a validated stable prefix, paired with <strong>PTI</strong>, a constrained structured local tool interface. On a selected 100-case BFCL-derived control ladder with a local Gemma 4 12B model, restored KV matched native live append at <code>100/100</code> with zero fresh-tail or wrong-capsule leaks; compact visible evidence solved <code>19/100</code>, and direct visible tools solved <code>91/100</code>. On a repeated-work stream, KV Capsule + PTI achieved <code>100/100</code> with <code>3,900</code> cumulative new visible tail/control tokens after state restore and <code>853,499 ms</code> cumulative wall time, compared with <code>86/100</code>, <code>245,774,883</code> cumulative reported visible input tokens, and <code>11,813,816 ms</code> cumulative wall time for the tested text-threaded local-agent baseline using the same model family. These results support a narrow systems claim: for the tested repeated local tool-use workloads, stable context can be preserved as reusable hidden state rather than repeatedly replayed or summarized as visible text.
      </div>
    </section>

    <section id="intro">
      <h2>1. Introduction</h2>
      <p>Local tool-using agents often carry stable context as visible text across many tasks. This context includes tool schemas, runtime rules, output contracts, and environment instructions. Tool-use benchmarks such as BFCL, ToolLLM, and Gorilla show that the exact representation of tool APIs and function-call contracts matters for reliability [<a class="cite" href="https://openreview.net/forum?id=2GmDdhBdDk" target="_blank" rel="noreferrer">3</a>,<a class="cite" href="https://arxiv.org/abs/2307.16789" target="_blank" rel="noreferrer">4</a>,<a class="cite" href="https://arxiv.org/abs/2305.15334" target="_blank" rel="noreferrer">5</a>]. When this context becomes too large, some practical runtimes, including the tested Codex/Ollama route, compact it into natural-language summaries. This makes the representation portable, but it also changes the object being reused.</p>
      ...
      <p>We test that premise with two runtime pieces. The first is a <strong>KV Capsule</strong>: a saved, quality-gated KV/sequence state after the model reads a stable tool/context prefix. The second is a constrained structured local tool interface, which we call <strong>PTI</strong>. PTI exposes a compact execution surface, letting the model emit structured calls while the stable interface rules live in the preserved prefix state. This differs from memory systems such as MemGPT and Reflexion, which reintroduce stored information as visible text or verbal feedback rather than restoring the model's computed prefix state [<a class="cite" href="https://arxiv.org/abs/2310.08560" target="_blank" rel="noreferrer">6</a>,<a class="cite" href="https://arxiv.org/abs/2303.11366" target="_blank" rel="noreferrer">7</a>].</p>
      <p>The evaluation uses BFCL-derived tasks [<a class="cite" href="https://openreview.net/forum?id=2GmDdhBdDk" target="_blank" rel="noreferrer">3</a>] with a Gemma 4 12B model blob installed through Ollama and executed for the KV lanes via a llama.cpp CUDA sequence-file route [<a class="cite" href="https://github.com/ggml-org/llama.cpp" target="_blank" rel="noreferrer">8</a>]. The goal is not to compare against hosted frontier systems. The goal is to isolate whether runtime state and tool-interface design can make a local model function better and more reliably on repeated tool-use work.</p>
      ...
      <ul>
        ...
        <li>A constrained structured tool-interface harness that separates local tool execution from repeated natural-language schema replay.</li>
    ...
    <section id="claim">
      <h2>Main Claim</h2>
      <p><strong>Thesis:</strong> repeated local-agent work can be improved by reusing validated attention state for stable context instead of re-sending or summarizing that context as visible text. Prior serving work establishes that KV state can be managed and reused as a systems object [<a class="cite" href="https://arxiv.org/abs/2309.06180" target="_blank" rel="noreferrer">1</a>,<a class="cite" href="https://arxiv.org/abs/2312.07104" target="_blank" rel="noreferrer">2</a>], while agent-memory work shows the practical need for continuity across long interactions [<a class="cite" href="https://arxiv.org/abs/2310.08560" target="_blank" rel="noreferrer">6</a>,<a class="cite" href="https://arxiv.org/abs/2303.11366" target="_blank" rel="noreferrer">7</a>]. If the runtime preserves stable context as a KV Capsule and exposes tools through PTI, the same local model can resume work with far less visible text and less repeated visible schema/rule replay.</p>
      <p>This is a systems property, not a general model leaderboard claim. The evaluation asks whether KV Capsule + PTI improves the reliability of repeated local tool-use work compared with the tested text-threaded Codex/Ollama route and selected-cohort ablations on a local Gemma 4 12B setup.</p>
      <div class="callout">
    ...
    <section id="method">
      ...
      <h3>Constrained Structured Tool Interface</h3>
      <p>PTI is the name used in this runtime for a constrained structured local tool interface. It gives the model a compact contract for emitting tool/function-call plans after capsule restore, without replaying the full stable tool/interface schema as fresh visible text. This design is motivated by function-calling benchmarks that expose tool selection, argument construction, and multi-call composition as distinct reliability problems [<a class="cite" href="https://openreview.net/forum?id=2GmDdhBdDk" target="_blank" rel="noreferrer">3</a>,<a class="cite" href="https://arxiv.org/abs/2307.16789" target="_blank" rel="noreferrer">4</a>,<a class="cite" href="https://arxiv.org/abs/2305.15334" target="_blank" rel="noreferrer">5</a>]. PTI is narrower than arbitrary programmatic tool calling: it is a controlled execution surface built for deterministic local-agent evaluation.</p>
    ...
    <section id="discussion">
      ...
      <p>KV Capsules alone are not the full story. If the tool interface remains a sprawling natural-language schema that must be interpreted differently every turn, state reuse has less to preserve. PTI gives the model a stable, structured local contract. In the selected cohort, full visible PTI passed <code>100/100</code>, while direct visible tools passed <code>91/100</code>. That does not prove PTI is universally superior, but it does show that interface shape materially affected reliability on this selected cohort, which is consistent with tool-use benchmark findings around function selection and argument construction [<a class="cite" href="https://openreview.net/forum?id=2GmDdhBdDk" target="_blank" rel="noreferrer">3</a>,<a class="cite" href="https://arxiv.org/abs/2307.16789" target="_blank" rel="noreferrer">4</a>,<a class="cite" href="https://arxiv.org/abs/2305.15334" target="_blank" rel="noreferrer">5</a>].</p>
    ...
    <section id="figures">
      ...
      <div class="figure">
        <div class="figure-head"><b>Figure 3.</b> Selected-cohort latency boundary for positive/comparison lanes. Lower is better; this experiment supports semantic preservation, not a speedup claim for restored KV.</div>
        <div class="figure-body">
          {figure_img("figure-02-latency", "Matplotlib horizontal bar chart of selected-cohort mean total latency for positive and comparison lanes.")}
        </div>
        ...
        <p class="caption">Restored KV is correct but slower than full-visible and native-live append in this mechanism run. Fresh-tail and wrong-capsule negative-control latency lanes are omitted from this chart; their leak counts are reported in RQ2.</p>
      </div>
      <div class="figure">
      ...
      <div class="figure">
        <div class="figure-head"><b>Figure 7.</b> Repeated-work output-token burden. Lower means fewer emitted tokens, but this is not quality-normalized efficiency because lane success differs.</div>
        <div class="figure-body">
      ...
      <div class="figure">
        <div class="figure-head"><b>Figure 8.</b> Initial full BFCL non-live official-evaluator check. Category accuracies are shown for the first exported prediction run; the dashed line marks BFCL Non-Live AST Acc.</div>
        <div class="figure-body">
          {figure_img("figure-07-initial-official-bfcl", "Matplotlib horizontal bar chart showing initial official BFCL non-live category accuracies and 69.88 percent BFCL Non-Live AST Acc.")}
        </div>
        <p class="caption">This chart records the first local official-evaluator calibration result. BFCL-style Non-Live AST Acc is <code>69.88%</code>; scoreable generated-record accuracy including irrelevance is <code>969/1390</code> (<code>69.71%</code>). It should be replaced by the newer full official run once that run is scored.</p>
      </div>
      <div class="figure">
    ...
    <section id="calibration">
      ...
      <p>The following BFCL non-live results are included to calibrate the export/evaluator path for the disclosed runtime. They are not part of the main selected-cohort mechanism claim, are not a public leaderboard submission, and should be superseded by a newer full official run once scored.</p>
      <h3>Initial Full BFCL Non-Live Check</h3>
      <p>The initial full non-live evaluator check scored <code>69.88%</code> on BFCL's leaderboard-style <strong>Non-Live AST Acc</strong> calculation. That score is an unweighted average over the non-live AST groups: simple, multiple, parallel, and parallel-multiple. The scoreable generated-record count across all seven local non-live files was <code>969/1390</code>, or <code>69.71%</code>, with irrelevance reported separately at <code>57.08%</code>.</p>
      ...
      <p>The official BFCL cost and latency columns are shown for public leaderboard rows. The public table does not report token/context-burden telemetry for those rows. The visible-input cell for our local harness uses the matching initial official BFCL run records, not the separate repeated-work stream: across <code>1390</code> generated records, the restored-KV route exposed <code>45,332</code> fresh task-tail tokens after restore, or <code>32.6</code> mean visible task-tail tokens per record. The same official-run packet records <code>159,883</code> stable-prefix tokens represented by the per-case capsule and <code>205,215</code> full-prompt-equivalent tokens. Our BFCL run was local pre-generated inference, so there is no measured API spend to report. The table's latency cells are calibration measurements, not hosted BFCL serving metrics.</p>
    </section>
    ...
    <section id="limitations">
      ...
      <ul>
        ...
        <li>Fresh-tail and wrong-capsule controls test leakage and prefix dependence only on this selected cohort; they do not establish general safety of hidden-state reuse.</li>
        <li>The <code>89</code> compaction events are supported by tracked summary artifacts, but the raw Windows Codex session JSONL is not present in this checkout.</li>
    ...
    <section id="artifacts">
      <h2>Artifacts and Data Availability</h2>
      <p>All paper-facing artifacts, analysis scripts, figure manifests, and arXiv sources are available in the public repository at <a href="https://github.com/fraction12/ai-research-lab">github.com/fraction12/ai-research-lab</a>. The selected-cohort mechanism results are under <code>research/02-quality-gated-stateful-kv-reuse/experiments/paper-grade-code-mode-kv-capsule-evaluation-2026-06-05/</code>, including <code>selected-cohort-summary.json</code> and <code>selected-cohort-findings.md</code>. The repeated-work comparison is in the same directory, including <code>repeated-work-speed-summary.json</code> and <code>repeated-work-speed-findings.md</code>. The BFCL calibration artifacts are under <code>research/02-quality-gated-stateful-kv-reuse/experiments/bfcl-official-kv-pti-leaderboard-lane/</code>, including <code>official-evaluator-summary-2026-06-07.md</code>, <code>official-context-burden-summary-2026-06-07.md</code>, and <code>official-latency-derivation-2026-06-07.md</code>. Figure provenance is recorded in <code>research/02-quality-gated-stateful-kv-reuse/paper-artifacts/figures/figure-manifest.json</code>, and the exact LaTeX source for this draft is under <code>research/02-quality-gated-stateful-kv-reuse/arxiv/</code>.</p>
    </section>
